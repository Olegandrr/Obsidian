...
this.getName = () => this.name создает метод getName для созданного объекта. И поскольку используется функция arrow, это значение внутри функции arrow равно значению внешней области видимости — функции конструктора Pet.  
  
Вызов cat.getName(), а также getName() возвращает выражение this.name это оценивается как "Пушистый".
